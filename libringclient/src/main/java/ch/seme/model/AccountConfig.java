...
public class AccountConfig {
    ...
    private final Map<ch.seme.model.ConfigKey, String> mValues;
    ...
    public AccountConfig(Map<String, String> details) {
        if (details != null) {
            ...
            for (Map.Entry<String, String> entry : details.entrySet()) {
                ch.seme.model.ConfigKey confKey = ch.seme.model.ConfigKey.fromString(entry.getKey());
                if (confKey != null) {
    ...
    public String get(ch.seme.model.ConfigKey key) {
        return mValues.get(key) != null ? mValues.get(key) : "";
    }

    public boolean getBool(ch.seme.model.ConfigKey key) {
        return TRUE_STR.equals(get(key));
    }

    public HashMap<String, String> getAll() {
        HashMap<String, String> details = new HashMap<>(mValues.size());
        for (Map.Entry<ch.seme.model.ConfigKey, String> entry : mValues.entrySet()) {
            details.put(entry.getKey().key(), entry.getValue());
        }
        return details;
    }

    void put(ch.seme.model.ConfigKey key, String value) {
        mValues.put(key, value);
    }

    void put(ch.seme.model.ConfigKey key, boolean value) {
        mValues.put(key, value ? TRUE_STR : FALSE_STR);
    }

    public Set<ch.seme.model.ConfigKey> getKeys() {
