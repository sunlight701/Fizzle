...
public class Account {
    ...
    private final Map<String, ch.seme.model.TrustRequest> mRequests = new HashMap<>();
    ...
    private final BehaviorSubject<Collection<ch.seme.model.TrustRequest>> trustRequestsSubject = BehaviorSubject.create();

    public boolean canSearch() {
        return !ch.seme.utils.StringUtils.isEmpty(getDetail(ch.seme.model.ConfigKey.MANAGER_URI));
    }
    ...
    private Single<ch.seme.utils.Tuple<String, Object>> mLoadedProfile = null;
    ...
    public void updated(Conversation conversation) {
        ...
        if (conversation == conversations.get(key))
            ...
            if (mContacts.containsKey(key) || !isFizzle()) {
    ...
    public void addTextMessage(TextMessage txt) {
        ...
        if (daemonId != null && !ch.seme.utils.StringUtils.isEmpty(daemonId)) {
    ...
    public CallContact getContactFromCache(String key) {
        if (ch.seme.utils.StringUtils.isEmpty(key))
            return null;
    ...
    public void setDetails(Map<String, String> details) {
        ...
        mUsername = mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_USERNAME);
    }

    public void setDetail(ch.seme.model.ConfigKey key, String val) {
        mDetails.put(key, val);
    }

    public void setDetail(ch.seme.model.ConfigKey key, boolean val) {
        mDetails.put(key, val);
    }
    ...
    public String getDisplayname() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_DISPLAYNAME);
    }
    ...
    public String getHost() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_HOSTNAME);
    }

    public void setHost(String host) {
        mDetails.put(ch.seme.model.ConfigKey.ACCOUNT_HOSTNAME, host);
    }

    public String getProxy() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_ROUTESET);
    }

    public void setProxy(String proxy) {
        mDetails.put(ch.seme.model.ConfigKey.ACCOUNT_ROUTESET, proxy);
    }

    public boolean isDhtProxyEnabled() {
        return mDetails.getBool(ch.seme.model.ConfigKey.PROXY_ENABLED);
    }

    public void setDhtProxyEnabled(boolean active) {
        mDetails.put(ch.seme.model.ConfigKey.PROXY_ENABLED, active ? "true" : "false");
    }

    public String getRegistrationState() {
        return mVolatileDetails.get(ch.seme.model.ConfigKey.ACCOUNT_REGISTRATION_STATUS);
    }

    public void setRegistrationState(String registeredState, int code) {
        mVolatileDetails.put(ch.seme.model.ConfigKey.ACCOUNT_REGISTRATION_STATUS, registeredState);
        mVolatileDetails.put(ch.seme.model.ConfigKey.ACCOUNT_REGISTRATION_STATE_CODE, Integer.toString(code));
    }
    ...
    public String getRegisteredName() {
        return mVolatileDetails.get(ch.seme.model.ConfigKey.ACCOUNT_REGISTERED_NAME);
    }

    public String getAlias() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_ALIAS);
    }

    public Boolean isSip() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_TYPE).equals(AccountConfig.ACCOUNT_TYPE_SIP);
    }

    public Boolean isFizzle() {
        return mDetails.get(ch.seme.model.ConfigKey.ACCOUNT_TYPE).equals(AccountConfig.ACCOUNT_TYPE_RING);
    }

    public void setAlias(String alias) {
        mDetails.put(ch.seme.model.ConfigKey.ACCOUNT_ALIAS, alias);
    }

    private String getDetail(ch.seme.model.ConfigKey key) {
        return mDetails.get(key);
    }

    public boolean getDetailBoolean(ch.seme.model.ConfigKey key) {
        return mDetails.getBool(key);
    }

    public boolean isEnabled() {
        return mDetails.getBool(ch.seme.model.ConfigKey.ACCOUNT_ENABLE);
    }

    public boolean isActive() {
        return mVolatileDetails.getBool(ch.seme.model.ConfigKey.ACCOUNT_ACTIVE);
    }

    public void setEnabled(boolean isChecked) {
        mDetails.put(ch.seme.model.ConfigKey.ACCOUNT_ENABLE, isChecked);
    }

    public boolean hasPassword() {
        return mDetails.getBool(ch.seme.model.ConfigKey.ARCHIVE_HAS_PASSWORD);
    }

    public boolean hasManager() {
        return !mDetails.get(ch.seme.model.ConfigKey.MANAGER_URI).isEmpty();
    }
    ...
    public boolean isAutoanswerEnabled() {
        return mDetails.getBool(ch.seme.model.ConfigKey.ACCOUNT_AUTOANSWER);
    }
    ...
    public String getDeviceId() {
        return getDetail(ch.seme.model.ConfigKey.ACCOUNT_DEVICE_ID);
    }
    ...
    public void addContact(String id, boolean confirmed) {
        ...
        ch.seme.model.TrustRequest req = mRequests.get(id);
        if (req != null) {
            mRequests.remove(id);
    ...
    private void addContact(Map<String, String> contact) {
        ...
        if (!ch.seme.utils.StringUtils.isEmpty(addedStr)) {
    ...
    public List<ch.seme.model.TrustRequest> getRequests() {
        ArrayList<ch.seme.model.TrustRequest> requests = new ArrayList<>(mRequests.size());
        for (ch.seme.model.TrustRequest request : mRequests.values()) {
    ...
    public Map<String, ch.seme.model.TrustRequest> getRequestsMigration() {
        return mRequests;
    }

    public ch.seme.model.TrustRequest getRequest(Uri uri) {
        return mRequests.get(uri.getRawRingId());
    }

    public void addRequest(ch.seme.model.TrustRequest request) {
    ...
    public void setRequests(List<ch.seme.model.TrustRequest> requests) {
        ch.seme.utils.Log.w(TAG, "setRequests " + requests.size());
        synchronized (pending) {
            for (ch.seme.model.TrustRequest request : requests) {
                mRequests.put(request.getContactId(), request);
                String key = new Uri(request.getContactId()).getUri();
                Conversation conversation = pending.get(key);
                if (conversation == null) {
    ...
    public void setHistoryLoaded(List<Conversation> conversations) {
        if (historyLoaded)
            return;
        ch.seme.utils.Log.w(TAG, "setHistoryLoaded() " + conversations.size());
        for (Conversation c : conversations)
            updated(c);
        historyLoaded = true;
        conversationChanged();
    ...
    private List<Conversation> getSortedConversations() {
        ch.seme.utils.Log.w(TAG, "getSortedConversations() " + Thread.currentThread().getId());
        synchronized (sortedConversations) {
            if (conversationsChanged) {
    ...
    synchronized public long onLocationUpdate(AccountService.Location location) {
        ch.seme.utils.Log.w(TAG, "onLocationUpdate " + location.getPeer() + " " + location.getLatitude() + ",  " + location.getLongitude());
    ...
    synchronized private void forceExpireContact(CallContact contact) {
        ch.seme.utils.Log.w(TAG, "forceExpireContact " + contactLocations.size());
        Observable<ContactLocation> cl = contactLocations.remove(contact);
        if (cl != null) {
            ch.seme.utils.Log.w(TAG, "Contact stopped sharing location: " + contact.getDisplayName());
    ...
    synchronized public void maintainLocation() {
        ch.seme.utils.Log.w(TAG, "maintainLocation " + contactLocations.size());
        ...
        while (it.hasNext())  {
            ...
            if (e.getValue().blockingFirst().receivedDate.before(expiration)) {
                ch.seme.utils.Log.w(TAG, "maintainLocation clearing " + e.getKey().getDisplayName());
    ...
    public Observable<Observable<ContactLocation>> getLocationUpdates(Uri contactId) {
        ...
        ch.seme.utils.Log.w(TAG, "getLocationUpdates " + contactId + " " + contact);
        return mLocationSubject
    ...
    public Single<String> getAccountAlias() {
        if (isFizzle()) {
            if (mLoadedProfile == null)
                return Single.just(getFizzleAlias());
            return mLoadedProfile.map(p -> ch.seme.utils.StringUtils.isEmpty(p.first) ? getFizzleAlias() : p.first);
        } else {
            if (mLoadedProfile == null)
                return Single.just(getAlias());
            return mLoadedProfile.map(p -> ch.seme.utils.StringUtils.isEmpty(p.first) ? getAlias() : p.first);
        }
    }
    ...
    public Single<ch.seme.utils.Tuple<String, Object>> getLoadedProfile() {
