...
import ch.seme.model.ConversationHistory;
...
public abstract class HistoryService {
    ...
    protected abstract Dao<ch.seme.model.Interaction, Integer> getInteractionDataDao(String dbName);

    protected abstract Dao<ch.seme.model.ConversationHistory, Integer> getConversationDataDao(String dbName);
    ...
    public Completable clearHistory(final String contactId, final String accountId, boolean deleteConversation) {
        ...
        return Completable.fromAction(() -> {
            ...
            ch.seme.model.ConversationHistory conversation = getConversationDataDao(accountId).queryBuilder()
                    .where().eq(ch.seme.model.ConversationHistory.COLUMN_PARTICIPANT, contactId).queryForFirst();

            if (conversation == null)
                return;

            DeleteBuilder<ch.seme.model.Interaction, Integer> deleteBuilder = getInteractionDataDao(accountId).deleteBuilder();
            if (deleteConversation) {
                // complete delete, remove conversation and all interactions
                deleteBuilder.where().eq(ch.seme.model.Interaction.COLUMN_CONVERSATION, conversation.getId());
                getConversationDataDao(accountId).deleteById(conversation.getId());
            } else {
                // keep conversation and contact event interactions
                deleteBuilder.where()
                        .eq(ch.seme.model.Interaction.COLUMN_CONVERSATION, conversation.getId()).and()
                        .ne(ch.seme.model.Interaction.COLUMN_TYPE, ch.seme.model.Interaction.InteractionType.CONTACT.toString());
            }

            deleted += deleteBuilder.delete();

            ch.seme.utils.Log.w(TAG, "clearHistory: removed " + deleted + " elements");
        }).subscribeOn(scheduler);
    }
     ...
     */
    public Completable clearHistory(List<ch.seme.model.Account> accounts) {
        return Completable.fromAction(() -> {
    ...
    public Completable updateInteraction(ch.seme.model.Interaction interaction, String accountId) {
     ...
     */
    public Completable insertInteraction(String accountId, Conversation conversation, ch.seme.model.Interaction interaction) {
        return Completable.fromAction(() -> {
            ch.seme.utils.Log.d(TAG, "Inserting interaction for account -> " + accountId);
            Dao<ch.seme.model.ConversationHistory, Integer> conversationDataDao = getConversationDataDao(accountId);

            ch.seme.model.ConversationHistory history = conversationDataDao.queryBuilder().where().eq(ch.seme.model.ConversationHistory.COLUMN_PARTICIPANT, conversation.getParticipant()).queryForFirst();

            if (history == null) {
                history = conversationDataDao.createIfNotExists(new ch.seme.model.ConversationHistory(conversation.getParticipant()));
            ...
            getInteractionDataDao(accountId).create(interaction);
                ...
                .doOnError(e -> ch.seme.utils.Log.e(TAG, "Can't insert interaction", e))
                .subscribeOn(scheduler);
    }
     ...
     */
    public Single<List<ch.seme.model.Interaction>> getSmartlist(final String accountId) {
        ch.seme.utils.Log.d(TAG, "Loading smartlist");
        return Single.fromCallable(() ->
                ...
                getInteractionDataDao(accountId).queryRaw("SELECT * FROM (SELECT DISTINCT id, author, conversation, MAX(timestamp), body, type, status, daemon_id, is_read, extra_data from interactions GROUP BY interactions.conversation) as final\n" +
                        ...
                        "GROUP BY final.conversation\n", (columnNames, resultColumns) -> new ch.seme.model.Interaction(resultColumns[0],
                        resultColumns[1], new ch.seme.model.ConversationHistory(Integer.parseInt(resultColumns[2]), resultColumns[12]), resultColumns[3], resultColumns[4], resultColumns[5], resultColumns[6], resultColumns[7], resultColumns[8], resultColumns[9])).getResults())
                .subscribeOn(scheduler).doOnError(e -> ch.seme.utils.Log.e(TAG, "Can't load smartlist from database", e))
     ...
     */
    public Single<List<ch.seme.model.Interaction>> getConversationHistory(final String accountId, final int conversationId) {
        ch.seme.utils.Log.d(TAG, "Loading conversation history:  Account ID -> " + accountId + ", ConversationID -> " + conversationId);
        return Single.fromCallable(() -> {
            Dao<ch.seme.model.Interaction, Integer> interactionDataDao = getInteractionDataDao(accountId);

            return interactionDataDao.query(interactionDataDao.queryBuilder()
                    .orderBy(ch.seme.model.Interaction.COLUMN_TIMESTAMP, true)
                    .where().eq(ch.seme.model.Interaction.COLUMN_CONVERSATION, conversationId)
                    .prepare());

        }).subscribeOn(scheduler).doOnError(e -> ch.seme.utils.Log.e(TAG, "Can't load conversation from database", e))
                .onErrorReturn(e -> new ArrayList<>());
    }

    Single<ch.seme.model.TextMessage> incomingMessage(final String accountId, final String daemonId, final String from, final String message) {
        return Single.fromCallable(() -> {
            String f = new ch.seme.model.Uri(from).getUri();
            Dao<ch.seme.model.ConversationHistory, Integer> conversationDataDao = getConversationDataDao(accountId);

            ch.seme.model.ConversationHistory conversation = conversationDataDao.queryBuilder().where().eq(ch.seme.model.ConversationHistory.COLUMN_PARTICIPANT, f).queryForFirst();
            ...
            ch.seme.model.TextMessage txt = new ch.seme.model.TextMessage(f, accountId, daemonId, conversation, message);
            txt.setStatus(ch.seme.model.Interaction.InteractionStatus.SUCCESS);
    ...
    Single<ch.seme.model.TextMessage> accountMessageStatusChanged(String accountId, long daemonId, String to, int status) {
        return Single.fromCallable(() -> {
            List<ch.seme.model.Interaction> textList = getInteractionDataDao(accountId).queryForEq(ch.seme.model.Interaction.COLUMN_DAEMON_ID, Long.toString(daemonId));
            ...
            ch.seme.model.TextMessage msg = new TextMessage(text);
