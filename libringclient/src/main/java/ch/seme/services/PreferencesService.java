...
public abstract class PreferencesService {
    ...
    private ch.seme.model.Settings mUserSettings;
    private final Subject<ch.seme.model.Settings> mSettingsSubject = BehaviorSubject.create();

    protected abstract ch.seme.model.Settings loadSettings();
    protected abstract void saveSettings(ch.seme.model.Settings settings);

    public ch.seme.model.Settings getSettings() {
        if (mUserSettings == null) {
            mUserSettings = loadSettings();
            mSettingsSubject.onNext(mUserSettings);
        }
        return new ch.seme.model.Settings(mUserSettings);
    }

    public void setSettings(ch.seme.model.Settings settings) {
    ...
    protected ch.seme.model.Settings getUserSettings() {
