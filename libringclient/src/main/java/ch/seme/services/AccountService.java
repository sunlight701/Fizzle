...
import ch.seme.model.Codec;
import ch.seme.model.ConfigKey;
import ch.seme.model.ContactEvent;
import ch.seme.model.Conversation;
import ch.seme.model.DataTransfer;
import ch.seme.model.DataTransferError;
...
import ch.seme.utils.SwigNativeConverter;
...
public class AccountService {
    ...
    @Inject
    ch.seme.services.HistoryService mHistoryService;

    @Inject
    DeviceRuntimeService mDeviceRuntimeService;

    @Inject
    ch.seme.services.VCardService mVCardService;

    private ch.seme.model.Account mCurrentAccount;
    private List<ch.seme.model.Account> mAccountList = new ArrayList<>();
    private boolean mHasSipAccount;
    private boolean mHasRingAccount;

    private final HashMap<Long, ch.seme.model.DataTransfer> mDataTransfers = new HashMap<>();
    private ch.seme.model.DataTransfer mStartingTransfer = null;

    private final BehaviorSubject<List<ch.seme.model.Account>> accountsSubject = BehaviorSubject.create();
    private final Subject<ch.seme.model.Account> accountSubject = PublishSubject.create();
    private final Observable<ch.seme.model.Account> currentAccountSubject = accountsSubject
            .filter(l -> !l.isEmpty())
            .map(l -> l.get(0))
    ...
    public static class Location {
        ...
        public ch.seme.model.Uri getPeer() {
            return peer;
        }
    ...
    private final Observable<ch.seme.model.TextMessage> incomingTextMessageSubject = incomingMessageSubject
            .flatMapMaybe(msg -> {
                String message = msg.messages.get(ch.seme.services.CallService.MIME_TEXT_PLAIN);
                if (message != null) {
    ...
    private final Observable<Location> incomingLocationSubject = incomingMessageSubject
            .flatMapMaybe(msg -> {
                try {
                    ...
                    l.callId = msg.callId;
                    l.peer = new ch.seme.model.Uri(msg.author);
                    return Maybe.just(l);
                } catch (Exception e) {
                    ch.seme.utils.Log.w(TAG, "Failed to receive geolocation", e);
                    return Maybe.empty();
                }
            })
            .share();

    private final Subject<ch.seme.model.TextMessage> textMessageSubject = PublishSubject.create();
    private final Subject<ch.seme.model.DataTransfer> dataTransferSubject = PublishSubject.create();
    private final Subject<ch.seme.model.TrustRequest> incomingRequestsSubject = PublishSubject.create();
    ...
    public static class UserSearchResult {
        ...
        public int state;
        public List<ch.seme.model.CallContact> results;
        ...
        public List<Observable<SmartListViewModel>> getResultsViewModels() {
            ...
            for (ch.seme.model.CallContact user : results) {
    ...
    public Observable<ch.seme.model.TextMessage> getIncomingMessages() {
    ...
    public Observable<ch.seme.model.TrustRequest> getIncomingRequests() {
    ...
    private void refreshAccountsCacheFromDaemon() {
        ch.seme.utils.Log.w(TAG, "refreshAccountsCacheFromDaemon");
        boolean hasSip = false, hasFizzle = false;
        List<ch.seme.model.Account> curList = mAccountList;
        List<String> accountIds = new ArrayList<>(Ringservice.getAccountList());
        List<ch.seme.model.Account> newAccounts = new ArrayList<>(accountIds.size());
        for (String id : accountIds) {
            for (ch.seme.model.Account acc : curList)
        ...
        for (String accountId : accountIds) {
            ch.seme.model.Account account = getAccount(accountId);
            ...
            if (account == null) {
                account = new ch.seme.model.Account(accountId, details, credentials, volatileAccountDetails);
            ...
            if (account.isSip()) {
                ...
                for (Map<String, String> requestInfo : requests) {
                    ch.seme.model.TrustRequest request = new ch.seme.model.TrustRequest(accountId, requestInfo);
                    account.addRequest(request);
                    ch.seme.model.CallContact contact = account.getContactFromCache(request.getContactId());
                ...
                if (enabled) {
                    for (ch.seme.model.CallContact contact : account.getContacts().values()) {
        ...
        mHasRingAccount = hasFizzle;
        if (!newAccounts.isEmpty()) {
            ch.seme.model.Account newAccount = newAccounts.get(0);
        ...
        }, e -> ch.seme.utils.Log.e(TAG, "Error completing profile migration", e));

        accountsSubject.onNext(newAccounts);
    }

    private ch.seme.model.Account getAccountByName(final String name) {
        for (ch.seme.model.Account acc : mAccountList) {
     ...
     */
    public Observable<ch.seme.model.Account> addAccount(final Map<String, String> map) {
        return Observable.fromCallable(() -> {
            String accountId = Ringservice.addAccount(StringMap.toSwig(map));
            if (ch.seme.utils.StringUtils.isEmpty(accountId)) {
            ...
            if (account == null) {
                ...
                account = new ch.seme.model.Account(accountId, accountDetails, accountCredentials, accountVolatileDetails);
                account.setDevices(accountDevices);
                if (account.isSip()) {
                    account.setRegistrationState(ch.seme.model.AccountConfig.STATE_READY, -1);
                }
     ...
     */
    public ch.seme.model.Account getCurrentAccount() {
     ...
     */
    public void setCurrentAccount(ch.seme.model.Account currentAccount) {
        ...
        final List<ch.seme.model.Account> accounts = getAccounts();
        ...
        for (ch.seme.model.Account account : accounts) {
     ...
     */
    public ch.seme.model.Account getAccount(String accountId) {
        if (!ch.seme.utils.StringUtils.isEmpty(accountId)) {
            for (ch.seme.model.Account account : mAccountList)
    ...
    public Single<ch.seme.model.Account> getAccountSingle(final String accountId) {
        return accountsSubject
                ...
                .map(accounts -> {
                    ...
                    ch.seme.utils.Log.d(TAG, "getAccountSingle() can't find account " + accountId);
     ...
     */
    public List<ch.seme.model.Account> getAccounts() {
        return mAccountList;
    }

    public Observable<List<ch.seme.model.Account>> getObservableAccountList() {
        return accountsSubject;
    }

    private Single<List<ch.seme.model.Account>> loadAccountProfiles(List<ch.seme.model.Account> accounts) {
        if (accounts.isEmpty())
            return Single.just(accounts);
        List<Single<ch.seme.model.Account>> loadedAccounts = new ArrayList<>(accounts.size());
        for (ch.seme.model.Account account : accounts)
            loadedAccounts.add(loadAccountProfile(account));
        return Single.concatEager(loadedAccounts).toList(accounts.size());
    }

    public Observable<List<ch.seme.model.Account>> getProfileAccountList() {
        return accountsSubject.concatMapSingle(this::loadAccountProfiles);
    }

    private Single<ch.seme.model.Account> loadAccountProfile(ch.seme.model.Account account) {
        if (account.getProfile() == null)
            return ch.seme.utils.VCardUtils.loadLocalProfileFromDiskWithDefault(mDeviceRuntimeService.provideFilesDir(), account.getAccountID())
    ...
    public Subject<ch.seme.model.Account> getObservableAccounts() {
        return accountSubject;
    }

    public Observable<ch.seme.model.Account> getObservableAccountUpdates(String accountId) {
        return accountSubject.filter(acc -> acc.getAccountID().equals(accountId));
    }

    public Observable<ch.seme.model.Account> getObservableAccount(String accountId) {
        return Observable.fromCallable(() -> getAccount(accountId))
                .concatWith(getObservableAccountUpdates(accountId));
    }
    public Observable<ch.seme.model.Account> getObservableAccount(ch.seme.model.Account account) {
        return Observable.just(account)
                .concatWith(accountSubject.filter(acc -> acc == account));
    }

    public Observable<ch.seme.model.Account> getCurrentAccountSubject() {
    ...
    public void sendProfile(final String callId, final String accountId) {
        mVCardService.loadSmallVCard(accountId, VCardService.MAX_SIZE_SIP)
                ...
                .subscribe(vcard -> {
                    String stringVCard = ch.seme.utils.VCardUtils.vcardToString(vcard);
                    ...
                    ch.seme.utils.Log.d(TAG, "sendProfile, vcard " + stringVCard);

                    while (i <= nbTotal) {
                        HashMap<String, String> chunk = new HashMap<>();
                        ch.seme.utils.Log.d(TAG, "length vcard " + stringVCard.length() + " id " + key + " part " + i + " nbTotal " + nbTotal);
                        String keyHashMap = ch.seme.utils.VCardUtils.MIME_PROFILE_VCARD + "; id=" + key + ",part=" + i + ",of=" + nbTotal;
                ...
                }, e -> ch.seme.utils.Log.w(TAG, "Not sending empty profile", e));
    }
    ...
    public Map<String, String> getAccountDetails(final String accountId) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running getAccountDetails()", e);
        }
        return null;
    }
    ...
    public void setAccountDetails(final String accountId, final Map<String, String> map) {
        ch.seme.utils.Log.i(TAG, "setAccountDetails() " + accountId);
    ...
    public Single<String> migrateAccount(String accountId, String password) {
        return mMigrationSubject
                ...
                .firstOrError()
                .doOnSubscribe(s -> {
                    final ch.seme.model.Account account = getAccount(accountId);
                    HashMap<String, String> details = account.getDetails();
                    details.put(ch.seme.model.ConfigKey.ARCHIVE_PASSWORD.key(), password);
    ...
    public void setAccountsActive(final boolean active) {
        mExecutor.execute(() -> {
            ch.seme.utils.Log.i(TAG, "setAccountsActive() running... " + active);
            for (ch.seme.model.Account a : mAccountList) {
    ...
    public void setAccountsVideoEnabled(boolean isEnabled) {
        for (ch.seme.model.Account account : mAccountList) {
            account.setDetail(ch.seme.model.ConfigKey.VIDEO_ENABLED, isEnabled);
        }
    }
    ...
    public Map<String, String> getVolatileAccountDetails(final String accountId) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running getVolatileAccountDetails()", e);
        }
        return null;
    }
    ...
    public Single<HashMap<String, String>> getAccountTemplate(final String accountType) {
        ch.seme.utils.Log.i(TAG, "getAccountTemplate() " + accountType);
    ...
    public void removeAccount(final String accountId) {
        ch.seme.utils.Log.i(TAG, "removeAccount() " + accountId);
    ...
    public Single<String> exportOnRing(final String accountId, final String password) {
        return mExportSubject
                ...
                .doOnSubscribe(l -> {
                    ch.seme.utils.Log.i(TAG, "exportOnRing() " + accountId);
    ...
    public Map<String, String> getKnownRingDevices(final String accountId) {
        ch.seme.utils.Log.i(TAG, "getKnownRingDevices() " + accountId);
        try {
            return mExecutor.submit(() -> Ringservice.getKnownRingDevices(accountId).toNative()).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running getKnownRingDevices()", e);
        }
        return null;
    }
    ...
    public void renameDevice(final String accountId, final String newName) {
        final ch.seme.model.Account account = getAccount(accountId);
        mExecutor.execute(() -> {
            ch.seme.utils.Log.i(TAG, "renameDevice() thread running... " + newName);
            StringMap details = Ringservice.getAccountDetails(accountId);
            details.put(ch.seme.model.ConfigKey.ACCOUNT_DEVICE_NAME.key(), newName);
            Ringservice.setAccountDetails(accountId, details);
            account.setDetail(ch.seme.model.ConfigKey.ACCOUNT_DEVICE_NAME, newName);
            account.setDevices(Ringservice.getKnownRingDevices(accountId).toNative());
        });
    }
     ...
     */
    public Single<List<ch.seme.model.Codec>> getCodecList(final String accountId) {
        return Single.fromCallable(() -> {
            List<ch.seme.model.Codec> results = new ArrayList<>();
            ...
            for (int i = 0; i < payloads.size(); ++i) {
                ...
                if (details.size() > 1) {
                    ...
                    ch.seme.utils.Log.i(TAG, "Error loading codec " + i);
                }
            }
            return results;
        }).subscribeOn(Schedulers.from(mExecutor));
    ...
    public Map<String, String> validateCertificatePath(final String accountID, final String certificatePath, final String privateKeyPath, final String privateKeyPass) {
        try {
            return mExecutor.submit(() -> {
                ch.seme.utils.Log.i(TAG, "validateCertificatePath() running...");
                return Ringservice.validateCertificatePath(accountID, certificatePath, privateKeyPath, privateKeyPass, "").toNative();
            }).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running validateCertificatePath()", e);
        }
        return null;
    }
    ...
    public Map<String, String> validateCertificate(final String accountId, final String certificate) {
        try {
            return mExecutor.submit(() -> {
                ch.seme.utils.Log.i(TAG, "validateCertificate() running...");
                return Ringservice.validateCertificate(accountId, certificate).toNative();
            }).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running validateCertificate()", e);
        }
        return null;
    }
    ...
    public Map<String, String> getCertificateDetailsPath(final String certificatePath) {
        try {
            return mExecutor.submit(() -> {
                ch.seme.utils.Log.i(TAG, "getCertificateDetailsPath() running...");
                return Ringservice.getCertificateDetails(certificatePath).toNative();
            }).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running getCertificateDetailsPath()", e);
        }
        return null;
    }
    ...
    public Map<String, String> getCertificateDetails(final String certificateRaw) {
        try {
            return mExecutor.submit(() -> {
                ch.seme.utils.Log.i(TAG, "getCertificateDetails() running...");
                return Ringservice.getCertificateDetails(certificateRaw).toNative();
            }).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running getCertificateDetails()", e);
        }
        return null;
    }
    ...
    public List<String> getTlsSupportedMethods() {
        ch.seme.utils.Log.i(TAG, "getTlsSupportedMethods()");
        return ch.seme.utils.SwigNativeConverter.toJava(Ringservice.getSupportedTlsMethod());
    }
    ...
    public List<Map<String, String>> getCredentials(final String accountId) {
        try {
            return mExecutor.submit(() -> {
                ch.seme.utils.Log.i(TAG, "getCredentials() running...");
                return Ringservice.getCredentials(accountId).toNative();
            }).get();
        } catch (Exception e) {
            ch.seme.utils.Log.e(TAG, "Error running getCredentials()", e);
        }
        return null;
    }
    ...
    public void setCredentials(final String accountId, final List<Map<String, String>> credentials) {
        ch.seme.utils.Log.i(TAG, "setCredentials() " + accountId);
        mExecutor.execute(() -> Ringservice.setCredentials(accountId, ch.seme.utils.SwigNativeConverter.toSwig(credentials)));
    }
    ...
    public void registerAllAccounts() {
        ch.seme.utils.Log.i(TAG, "registerAllAccounts()");
    ...
    public int backupAccounts(final List<String> accountIds, final String toDir, final String password) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running backupAccounts()", e);
        }
        return 1;
    }
    ...
    public int restoreAccounts(final String archivePath, final String password) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running restoreAccounts()", e);
        }
        return 1;
    }
     ...
     */
    public void registerName(final ch.seme.model.Account account, final String password, final String name) {

        if (account.registeringUsername) {
            ch.seme.utils.Log.w(TAG, "Already trying to register username");
            return;
        }
    ...
    public void registerName(final String account, final String password, final String name) {
        ch.seme.utils.Log.i(TAG, "registerName()");
    ...
    public List<Map<String, String>> getTrustRequests(final String accountId) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running getTrustRequests()", e);
        }
        return null;
    }
     ...
     */
    public void acceptTrustRequest(final String accountId, final ch.seme.model.Uri from) {
        ch.seme.utils.Log.i(TAG, "acceptRequest() " + accountId + " " + from);
        ch.seme.model.Account account = getAccount(accountId);
        if (account != null) {
            ch.seme.model.TrustRequest request = account.getRequest(from);
            if (request != null) {
                VCard vCard = request.getVCard();
                if (vCard != null) {
                    ch.seme.utils.VCardUtils.savePeerProfileToDisk(vCard, accountId, from.getRawRingId() + ".vcf", mDeviceRuntimeService.provideFilesDir());
                }
            }
     ...
     */
    private void handleTrustRequest(String accountId, String contactUri, ch.seme.model.TrustRequest request, ContactType type) {
        ch.seme.model.ContactEvent event = new ch.seme.model.ContactEvent();
        switch (type) {
            case ADDED:
                break;
            case INVITATION_RECEIVED:
                event.setStatus(ch.seme.model.Interaction.InteractionStatus.UNKNOWN);
                event.setAuthor(contactUri);
                event.setTimestamp(request.getTimestamp());
                break;
            case INVITATION_ACCEPTED:
                event.setStatus(ch.seme.model.Interaction.InteractionStatus.SUCCESS);
                event.setAuthor(contactUri);
                break;
     ...
     */
    private void migrateContactEvents(ch.seme.model.Account account, Map<String, ch.seme.model.CallContact> contacts, Map<String, ch.seme.model.TrustRequest> requests) {
        String accountId = account.getAccountID();
        for (ch.seme.model.TrustRequest request : requests.values()) {
            ch.seme.model.CallContact contact = account.getContactFromCache(request.getContactId());
            ch.seme.model.ContactEvent event = new ch.seme.model.ContactEvent(contact, request);
            insertContactEvent(accountId, contact.getPrimaryUri().getUri(), event);
        }
        for (ch.seme.model.CallContact contact : contacts.values()) {
            ch.seme.model.ContactEvent event = new ch.seme.model.ContactEvent(contact);
            insertContactEvent(accountId, contact.getPrimaryUri().getUri(), event);
    ...
    private void insertContactEvent(String accountId, String participantUri, ContactEvent event) {
        ch.seme.model.Conversation conversation = getAccount(accountId).getByUri(participantUri);
     ...
     */
    public boolean discardTrustRequest(final String accountId, final ch.seme.model.Uri contact) {
        ch.seme.model.Account account = getAccount(accountId);
    ...
    public void sendTrustRequest(final String accountId, final String to, final Blob message) {
        ch.seme.utils.Log.i(TAG, "sendTrustRequest() " + accountId + " " + to);
        handleTrustRequest(accountId, new ch.seme.model.Uri(to).getUri(), null, ContactType.ADDED);
    ...
    public void addContact(final String accountId, final String uri) {
        ch.seme.utils.Log.i(TAG, "addContact() " + accountId + " " + uri);
        handleTrustRequest(accountId, new ch.seme.model.Uri(uri).getUri(), null, ContactType.ADDED);
    ...
    public void removeContact(final String accountId, final String uri, final boolean ban) {
        ch.seme.utils.Log.i(TAG, "removeContact() " + accountId + " " + uri + " ban:" + ban);
    ...
    public List<Map<String, String>> getContacts(final String accountId) {
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error running getContacts()", e);
        }
        return null;
    }
    ...
    public void lookupName(final String account, final String nameserver, final String name) {
        ch.seme.utils.Log.i(TAG, "lookupName() " + account + " " + nameserver + " " + name);
    ...
    public Single<RegisteredName> findRegistrationByName(final String account, final String nameserver, final String name) {
        if (ch.seme.utils.StringUtils.isEmpty(name)) {
    ...
    public Single<UserSearchResult> searchUser(final String account, final String query) {
        if (ch.seme.utils.StringUtils.isEmpty(query)) {
    ...
    void stunStatusFailure(String accountId) {
        ch.seme.utils.Log.d(TAG, "stun status failure: " + accountId);
    }

    void registrationStateChanged(String accountId, String newState, int code, String detailString) {
        ch.seme.utils.Log.d(TAG, "registrationStateChanged: " + accountId + ", " + newState + ", " + code + ", " + detailString);
        ...
        if (oldState.contentEquals(ch.seme.model.AccountConfig.STATE_INITIALIZING) &&
    ...
    void accountDetailsChanged(String accountId, Map<String, String> details) {
        ...
        ch.seme.utils.Log.d(TAG, "accountDetailsChanged: " + accountId + " " + details.size());
        account.setDetails(details);
        accountSubject.onNext(account);
    }
    ...
    void volatileAccountDetailsChanged(String accountId, Map<String, String> details) {
        ...
        ch.seme.utils.Log.d(TAG, "volatileAccountDetailsChanged: " + accountId + " " + details.size());
    ...
    public void accountProfileReceived(String accountId, String name, String photo) {
        ...
        mVCardService.saveVCardProfile(accountId, account.getUri(), name, photo)
                .subscribeOn(Schedulers.io())
                .subscribe(account::setProfile, e -> ch.seme.utils.Log.e(TAG, "Error saving profile", e));
    }
    ...
    void profileReceived(String accountId, String peerId, String vcardPath) {
        ...
        if (account == null)
            return;
        ch.seme.utils.Log.w(TAG, "profileReceived: " + accountId + ", " + peerId + ", " + vcardPath);
        ch.seme.model.CallContact contact = account.getContactFromCache(peerId);
        mVCardService.peerProfileReceived(accountId, peerId, new File(vcardPath))
                .subscribe(profile -> contact.setProfile(profile.first, profile.second), e -> ch.seme.utils.Log.e(TAG, "Error saving contact profile", e));
    }

    void incomingAccountMessage(String accountId, String messageId, String callId, String from, Map<String, String> messages) {
        ch.seme.utils.Log.d(TAG, "incomingAccountMessage: " + accountId + " " + messages.size());
    ...
    void accountMessageStatusChanged(String accountId, long messageId, String to, int status) {
        ch.seme.utils.Log.d(TAG, "accountMessageStatusChanged: " + accountId + ", " + messageId + ", " + to + ", " + status);
        mHistoryService
                .accountMessageStatusChanged(accountId, messageId, to, status)
                .subscribe(textMessageSubject::onNext, e -> ch.seme.utils.Log.e(TAG, "Error updating message: " + e.getLocalizedMessage()));
    }

    public void composingStatusChanged(String accountId, String contactUri, int status) {
        ch.seme.utils.Log.d(TAG, "composingStatusChanged: " + accountId + ", " + contactUri + " " + status);
        getAccountSingle(accountId)
                .subscribe(account -> account.composingStatusChanged(new ch.seme.model.Uri(contactUri), ch.seme.model.Account.ComposingStatus.fromInt(status)));
    }

    void errorAlert(int alert) {
        ch.seme.utils.Log.d(TAG, "errorAlert : " + alert);
    }

    void knownDevicesChanged(String accountId, Map<String, String> devices) {
        ch.seme.model.Account accountChanged = getAccount(accountId);
    ...
    void exportOnRingEnded(String accountId, int code, String pin) {
        ch.seme.utils.Log.d(TAG, "exportOnRingEnded: " + accountId + ", " + code + ", " + pin);
    ...
    void nameRegistrationEnded(String accountId, int state, String name) {
        ch.seme.utils.Log.d(TAG, "nameRegistrationEnded: " + accountId + ", " + state + ", " + name);

        ch.seme.model.Account acc = getAccount(accountId);
        if (acc == null) {
            ch.seme.utils.Log.w(TAG, "Can't find account for name registration callback");
            return;
        }
        ...
        if (state == 0) {
            acc.setDetail(ch.seme.model.ConfigKey.ACCOUNT_REGISTERED_NAME, name);
        }
    ...
    void migrationEnded(String accountId, String state) {
        ch.seme.utils.Log.d(TAG, "migrationEnded: " + accountId + ", " + state);
    ...
    void deviceRevocationEnded(String accountId, String device, int state) {
        ch.seme.utils.Log.d(TAG, "deviceRevocationEnded: " + accountId + ", " + device + ", " + state);
    ...
    void incomingTrustRequest(String accountId, String from, String message, long received) {
        ch.seme.utils.Log.d(TAG, "incomingTrustRequest: " + accountId + ", " + from + ", " + received);

        ch.seme.model.Account account = getAccount(accountId);
        if (account != null) {
            ch.seme.model.TrustRequest request = new TrustRequest(accountId, from, received * 1000L, message);
            VCard vcard = request.getVCard();
            if (vcard != null) {
                ch.seme.model.CallContact contact = account.getContactFromCache(request.getContactId());
                if (!contact.detailsLoaded) {
            ...
            account.addRequest(request);
            handleTrustRequest(accountId, new ch.seme.model.Uri(from).getUri(), request, ContactType.INVITATION_RECEIVED);
    ...
    void contactRemoved(String accountId, String uri, boolean banned) {
        ch.seme.model.Account account = getAccount(accountId);
        ch.seme.utils.Log.d(TAG, "Contact removed: " + uri + " User is banned: " + banned);
        if (account != null) {
    ...
    public ch.seme.model.DataTransferError sendFile(final ch.seme.model.DataTransfer dataTransfer, File file) {
        ...
        ch.seme.utils.Log.i(TAG, "sendFile() id=" + dataTransfer.getId() + " accountId=" + dataTransferInfo.getAccountId() + ", peer=" + dataTransferInfo.getPeer() + ", filePath=" + dataTransferInfo.getPath());
        try {
            return getDataTransferError(mExecutor.submit(() -> Ringservice.sendFile(dataTransferInfo, 0)).get());
        } catch (Exception ignored) {
        }
        return ch.seme.model.DataTransferError.UNKNOWN;
    }
    ...
    public void acceptFileTransfer(ch.seme.model.DataTransfer transfer) {
        if (transfer == null)
            return;
    ...
    private void acceptFileTransfer(final Long dataTransferId, final String filePath, final long offset) {
        ch.seme.utils.Log.i(TAG, "acceptFileTransfer() id=" + dataTransferId + ", path=" + filePath + ", offset=" + offset);
    ...
    public void cancelDataTransfer(final Long dataTransferId) {
        ch.seme.utils.Log.i(TAG, "cancelDataTransfer() id=" + dataTransferId);
        mExecutor.execute(() -> Ringservice.cancelDataTransfer(dataTransferId));
    }

    private class DataTransferRefreshTask implements Runnable {
        private final ch.seme.model.DataTransfer mToUpdate;
        public ScheduledFuture<?> scheduledTask;

        DataTransferRefreshTask(ch.seme.model.DataTransfer t) {
            mToUpdate = t;
        }
        ...
        public void run() {
            synchronized (mToUpdate) {
                if (mToUpdate.getStatus() == ch.seme.model.Interaction.InteractionStatus.TRANSFER_ONGOING) {
    ...
    void dataTransferEvent(final long transferId, int eventCode) {
        ch.seme.model.Interaction.InteractionStatus transferStatus = getDataTransferEventCode(eventCode);
        ch.seme.utils.Log.d(TAG, "Data Transfer " + transferStatus);
        DataTransferInfo info = new DataTransferInfo();
        if (getDataTransferError(Ringservice.dataTransferInfo(transferId, info)) != ch.seme.model.DataTransferError.SUCCESS)
            return;

        ch.seme.model.Account account = getAccount(info.getAccountId());
        Conversation c = account.getByUri(new Uri(info.getPeer()).getUri());

        boolean outgoing = info.getFlags() == 0;
        ch.seme.model.DataTransfer transfer = mDataTransfers.get(transferId);
        if (transfer == null) {
            ...
            if (outgoing && mStartingTransfer != null) {
                ...
                transfer = new ch.seme.model.DataTransfer(c, account.getAccountID(), info.getDisplayName(),
            ...
            ch.seme.model.Interaction.InteractionStatus oldState = transfer.getStatus();
            if (oldState != transferStatus) {
                if (transferStatus == ch.seme.model.Interaction.InteractionStatus.TRANSFER_ONGOING) {
                ...
                } else if (transferStatus == (ch.seme.model.Interaction.InteractionStatus.TRANSFER_FINISHED)) {
                    if (!transfer.isOutgoing()) {
                        File tmpPath = mDeviceRuntimeService.getTemporaryPath(transfer.getPeerId(), transfer.getStoragePath());
    ...
    private static ch.seme.model.Interaction.InteractionStatus getDataTransferEventCode(int eventCode) {
        ch.seme.model.Interaction.InteractionStatus dataTransferEventCode = ch.seme.model.Interaction.InteractionStatus.INVALID;
        try {
            dataTransferEventCode = ch.seme.model.Interaction.InteractionStatus.fromIntFile(eventCode);
        } catch (ArrayIndexOutOfBoundsException ignored) {
            ch.seme.utils.Log.e(TAG, "getEventCode: invalid data transfer status from daemon");
        }
        return dataTransferEventCode;
    }

    private static ch.seme.model.DataTransferError getDataTransferError(Long errorCode) {
        ch.seme.model.DataTransferError dataTransferError = ch.seme.model.DataTransferError.UNKNOWN;
        if (errorCode == null) {
            ch.seme.utils.Log.e(TAG, "getDataTransferError: invalid error code");
        } else {
            try {
                dataTransferError = DataTransferError.values()[errorCode.intValue()];
            } catch (ArrayIndexOutOfBoundsException ignored) {
                ch.seme.utils.Log.e(TAG, "getDataTransferError: invalid data transfer error from daemon");
            }
        }
        return dataTransferError;
    }

    public ch.seme.model.DataTransfer getDataTransfer(long id) {
        return mDataTransfers.get(id);
    }

    public Subject<ch.seme.model.DataTransfer> getDataTransfers() {
        return dataTransferSubject;
    }

    public Observable<ch.seme.model.DataTransfer> observeDataTransfer(DataTransfer transfer) {
        return dataTransferSubject
                .filter(t -> t == transfer)
                .startWith(transfer);
    }
    ...
    public void setProxyEnabled(boolean enabled) {
        mExecutor.execute(() -> {
            for (Account acc : mAccountList) {
                if (acc.isFizzle() && (acc.isDhtProxyEnabled() != enabled)) {
