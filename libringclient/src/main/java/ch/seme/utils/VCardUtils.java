...
public final class VCardUtils {
    ...
    public static ch.seme.utils.Tuple<String, byte[]> readData(VCard vcard) {
        ...
        if (vcard != null) {
            if (!vcard.getPhotos().isEmpty()) {
                try {
                    ...
                    ch.seme.utils.Log.w(TAG, "Can't read photo from VCard", e);
                    photo = null;
                }
            }
    ...
    private static void saveToDisk(VCard vcard, String filename, File path) {
        ...
        try (VCardWriter writer = new VCardWriter(file, VCardVersion.V2_1)) {
            ...
            ch.seme.utils.Log.e(TAG, "Error while saving VCard to disk", e);
        }
    }
    ...
    private static VCard loadFromDisk(File path) throws IOException {
        if (path == null || !path.exists()) {
            ch.seme.utils.Log.d(TAG, "vcardPath not exist " + path);
            return null;
        }
        if (path.length() > VCARD_MAX_SIZE) {
            ch.seme.utils.Log.w(TAG, "vcardPath too big: " + path.length() / 1024 + " kB");
            return null;
        }
    ...
    public static String vcardToString(VCard vcard) {
        ...
        try {
            ...
            ch.seme.utils.Log.e(TAG, "Error while converting VCard to String", e);
            stringVCard = null;
        }
