...
import ch.seme.R;
import ch.seme.about.AboutFragment;
...
import ch.seme.databinding.ActivityHomeBinding;
...
import ch.seme.settings.VideoSettingsFragment;
import ch.seme.settings.pluginssettings.PluginDetails;
import ch.seme.settings.pluginssettings.PluginPathPreferenceFragment;
import ch.seme.settings.pluginssettings.PluginSettingsFragment;
import ch.seme.settings.pluginssettings.PluginsListSettingsFragment;
import ch.seme.utils.ContentUriHandler;
import ch.seme.utils.ConversationPath;
import ch.seme.utils.DeviceUtils;
import ch.seme.views.SwitchButton;
...
public class HomeActivity extends AppCompatActivity implements BottomNavigationView.OnNavigationItemSelectedListener,
    ...
    private static final int NAVIGATION_CONTACT_REQUESTS = 2;
    private static final int NAVIGATION_CONVERSATIONS = 0;
    private static final int NAVIGATION_CONTACTS = 1;
    private static final int NAVIGATION_ACCOUNT = 3;
    ...
    public void startConversation(String conversationId) {
        mDisposable.add(mAccountService.getCurrentAccountSubject()
                .firstElement()
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(account -> startConversation(account.getAccountID(), new ch.seme.model.Uri(conversationId))));
    }
    public void startConversation(String accountId, ch.seme.model.Uri conversationId) {
        if (!DeviceUtils.isTablet(this)) {
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.navigation_contacts:
                if (fContent instanceof ContactListFragment) {
                    break;
                }
                popCustomBackStack();
                fContent = new ContactListFragment();
                getSupportFragmentManager().beginTransaction()
                        .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                        .replace(R.id.main_frame, fContent, HOME_TAG)
                        .setReorderingAllowed(true)
